...
**Estro provides a pre-configured setup for common assets like fonts and icons.**
...
Estro uses [the `src/assets/` directory](../src/assets/) to organise raw assets that do require a build step. You are responsible for the processing and importing of these assets. Estro provides a pre-configured setup for [fonts](#fonts) and [icons](#icons). These assets are eventually compiled to `dist/_astro/` and are configured to be served with immutable cache headers for performance.

## Fonts

Estro provides a setup for custom fonts to improve their loading performance. An example font (Archivo) is pre-configured in [`src/assets/fonts.ts`](../src/assets/fonts.ts). To add your own custom fonts:
...
That's it. Estro will now automatically preload the font files and include critical CSS for your custom fonts.

## Icons

Estro combines icons in a sprite and makes them available through the `<Icon>` component. To use an icon:
